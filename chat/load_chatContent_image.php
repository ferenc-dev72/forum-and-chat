...
<div class="chat-box-image" id="chatBox">
    ...
    <div class="icon-container">
       <!--  <button id="addLink">&#128279;</button> Link ikon -->
        ...
        &nbsp;&nbsp;&nbsp;
        <button id="sendMessage_image"><img src='/image/chat/icons/submit.png' alt='join' width='30' height='30'></button> <!-- Kuldes gomb -->
    </div>
</div>
...
/*chat---------------------------------------------------------------------------------------------------*/
 ...
 var userHash_chat = $('.image-page-text40').data('userhash');
...
if (typeof interval_lastactive_image === 'undefined') {
    var interval_lastactive_image;
}


if (typeof intervalId_image === 'undefined') {
    var intervalId_image;
}
...
$.ajax({
  ...
  if ( (typeof lastMessageId_image === 'undefined') || (lastMessageId_image == null) ) {
    lastMessageId_image=0;
  }
...
intervalId_image = setInterval(loadMessages, 3000);

interval_lastactive_image = setInterval(updateLastActive, 30000);
$.ajax({
...
$(document).ready(function () {
    ...
    document.getElementById('sendMessage_image').addEventListener('click', function () {
        sendMessage();
    });
});
...
if (response.last_message_id_image) {
  lastMessageId_image = response.last_message_id_image;
}
...
function loadMessages() {
  ...
  $.ajax({
    type: 'GET',
    url: '/image/chat/load_messages_image.php',
    data: { to_users_chats_id: targetUserId, uid: uid, last_message_id_image: lastMessageId_image },
    success: function(response) {
      var chatMessages = $('#chatMessages');
