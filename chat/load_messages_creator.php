...
$last_message_id_creator = isset($_GET['last_message_id_creator']) ? intval($_GET['last_message_id_creator']) : 0;
...
$stmt->bind_param("iiiii", $to_users_chats_id, $from_users_chats_id, $from_users_chats_id, $to_users_chats_id, $last_message_id_creator);
$stmt->execute();
$result = $stmt->get_result();


$messages = [];
$lastMessageId_creator = $last_message_id_creator; // Inicializ�l�s a kapott �rt�kkel
...
while ($row = $result->fetch_assoc()) {
    ...
    $lastMessageId_creator = $row['message_id']; // Friss�t�s minden �j �zenettel
}
...
$response = [
    'messages' => $messages,
    'last_message_id_creator' => $lastMessageId_creator
];
