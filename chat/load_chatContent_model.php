...
<div class="chat-box-image" id="chatBox">
    ...
    <div class="icon-container">
       <!--  <button id="addLink">&#128279;</button> Link ikon -->
        ...
        &nbsp;&nbsp;&nbsp;
        <button id="sendMessage_model"><img src='/image/chat/icons/submit.png' alt='join' width='30' height='30'></button> <!-- Kuldes gomb -->
    </div>
</div>
...
/*chat---------------------------------------------------------------------------------------------------*/
 ...
 var userHash_chat = $('.model-page-text43').data('userhash');
...
if (typeof interval_lastactive_model === 'undefined') {
    var interval_lastactive_model;
}


if (typeof intervalId_model === 'undefined') {
    var intervalId_model;
}
...
if ( (typeof lastMessageId_model === 'undefined') || (lastMessageId_model == null) ) {
    lastMessageId_model=0;
  }
...
intervalId_model = setInterval(loadMessages, 3000);

interval_lastactive_model = setInterval(updateLastActive, 30000);
$.ajax({
...
$(document).ready(function () {
    ...
    document.getElementById('sendMessage_model').addEventListener('click', function () {
        sendMessage();
    });
});
...
if (response.last_message_id_model) {
  lastMessageId_model = response.last_message_id_model;
}
...
$.ajax({
type: 'GET',
url: '/image/chat/load_messages_model.php',
...
success: function(messages) {
  ...
  var chatMessages = $('#chatMessages');
   ...
   chatMessages.empty(); // Töröljük a meglévő üzeneteket
            messages.forEach(function (msg) {
                // Ellenőrizzük, hogy system üzenet-e
                if (msg.message == "[SYSTEM]:LEFT" || msg.message == "[SYSTEM]:JOINED") {
                    ...
                    chatMessages.append(systemMessage); // System üzenet hozzáadása
                } else {
                    // Üzenet konténer
                    var messageWrapper = $('<div>').addClass('message-wrapper');

                    // Metaadatok
                    var metaDiv = $('<div>').addClass('message-meta').css({
                        'font-size': 'smaller',
                        'color': 'gray',
                    }).html(`(${msg.is_read ? 'Read' : 'Unread'}) - ${msg.timestamp}`);

                    // Üzenet maga
                    var messageDiv = $('<div>').addClass('message');

                    if (msg.from_users_chats_id == uid) {
                        messageWrapper.addClass('self'); // Saját üzenet
                        messageDiv.addClass('self');
                    } else {
                        messageWrapper.addClass('partner'); // Partner üzenet
                        messageDiv.addClass('partner');
                    }

                    var messageContent = $('<span>').html(msg.message);

                    messageDiv.append(messageContent); // Üzenet szöveg hozzáadása
                    messageWrapper.append(metaDiv);   // Metaadatok hozzáadása a wrapperhez
                    messageWrapper.append(messageDiv); // Üzenet hozzáadása a wrapperhez
                    chatMessages.append(messageWrapper); // Wrapper hozzáadása a fő konténerhez
                }
            });
...
function loadMessages() {
  ...
  $.ajax({
    type: 'GET',
    url: '/image/chat/load_messages_model.php',
    data: { to_users_chats_id: targetUserId, uid: uid, last_message_id_model: lastMessageId_model },
    success: function(response) {
      var chatMessages = $('#chatMessages');
