...
$last_message_id_model = isset($_GET['last_message_id_model']) ? intval($_GET['last_message_id_model']) : 0;
...
$stmt->bind_param("iiiii", $to_users_chats_id, $from_users_chats_id, $from_users_chats_id, $to_users_chats_id, $last_message_id_model);
$stmt->execute();
$result = $stmt->get_result();


$messages = [];
$lastMessageId_model = $last_message_id_model; // Inicializálás a kapott értékkel
...
while ($row = $result->fetch_assoc()) {
    ...
    $lastMessageId_model = $row['message_id']; // Frissítés minden új üzenettel
}
...
$response = [
    'messages' => $messages,
    'last_message_id_model' => $lastMessageId_model
];
